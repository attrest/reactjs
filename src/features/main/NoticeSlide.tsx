          ...
          {noticeList.map((notice, index) => (
            <SwiperSlide key={index} className="w-auto">
              <Link href={'/notice/detail/'+ notice.id} className="block border border-white py-7.5 px-6 w-[288px]">
                ...
                <p className="mt-4 w-[84%] min-h-12 text-ellipsis line-clamp-2 break-all text-base font-blod">{decode(notice.title)}</p>
                <span className="block mt-7.5 text-sm font-normal text-gray2 font-pretendard">{format(new Date(notice.modDt), 'yyyy.MM.dd')}</span>
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
