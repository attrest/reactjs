...
gsap.fromTo(".box-1", {y: 50}, {duration: 2, x: 500, y: 0});`}</pre>
        ...
        <h4>4. gsap.set()</h4>
        <p>
          애니메이션 효과 없이 즉시 요소의 속성 값을 변경합니다. 이 메서드는 초기 상태를 설정하거나 애니메이션 전에 특정
          속성을 적용할 때 유용합니다. 여러개 속성에 동시에 설정이 가능하지만, 페이지 로딩 후 바로 보여야 하는
...
gsap.to(".sc_intro", {
    ...
    scale: 1.5,
        ...
        <p>
          ...
          <code>.restart()</code>는 특정 애니메이션의 재생, 일시 정지, 재개, 반전 및 재시작을 제어합니다. 이 메서드들은
          애니메이션의 흐름을 세밀하게 조정할 수 있게 해줍니다.
        </p>
        <div>
  ...
  gsap.to(text, {
    ...
    scrollTrigger: {
      ...
      <div>
        ...
        <p>
          웹 애니메이션은 사용자의 경험을 풍부하게 하고 웹사이트의 동적인 요소를 강화하는데, 이를 위해서는 애니메이션이
          사용자에게 정확한 시점에 표시되는 것이 중요합니다. GSAP는 이를 위한 강력한 도구를 제공하고 있는데, 여기에서는
          GSAP의 ScrollTrigger를 활용하여 스크롤에 따라 애니메이션을 정확히 제어하는 방법에 대해 설명합니다.
        </p>
        ...
        <p>
          ...
          <span>
            ScrollTrigger는 GSAP과 함께 사용되어, 사용자가 페이지를 스크롤할 때 정확한 위치에서 애니메이션이 시작되도록
            합니다.
          </span>
        </p>
        <p>
          스크롤 이벤트를 사용하기 위해선&nbsp;상단에&nbsp;<code>gsap.registerPlugin(ScrollTrigger);</code>로 스크롤
          트리거 플러그인을 불러오고 gsap 애니메이션 내부에 <code>scrollTrigger</code> 속성을 추가하여 애니메이션 효과를
          줄 요소를 등록해주어야 합니다.
        </p>
...
gsap.to(".box1", {
  ...
  x: 400
        ...
        <p>
          Pinning은 스크롤 동안 특정 요소를 화면에 고정시키는 기능입니다. 이를 통해 사용자의 주의를 끌거나 중요한 정보를
          강조할 수 있습니다. 사용법은 <code>pin: true</code> 또는 <code>{`pin: "Class/ID"`}</code>입니다.
        </p>
