...
export const useReviewsList = (page?:number, searchKeyword?:string, searchType?:string) => {
  ...
  return useQuery<DataItem<ReviewItem>>({
    ...
    queryFn: query,
    select: (data) => {
      return {...data, content: data.content.map((i, index) => {
        return {...i, 
          num: data.totalElements - (data.size * data.number) - index
        }
      })}
    },
...
export const useReviews = () => {
  ...
  const authChk = async (condition: {id:string, password:string}) => {
    const response = await Api.post<any>(`${path}/${condition.id}/password-verification`, {password:condition.password})
    return response.data
  }
